...
import { getUserIdForTodoById, saveTodo, updateTodo } from "../queries/todoQueries.ts";
...
export const insertTodo = async ({ title, description }: Todo, userId: string) => {
  const result = await saveTodo(title, description, userId);
  return result
};

export const editTodo = async ({ title, description }: Todo, userId: string, noteId: string) => {
